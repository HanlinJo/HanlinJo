...
class V2XRLEnvironment(gym.Env):
    ...
    #         for subchannel, users in subchannel_usage.items():
    #             # 检查是否有攻击者参与
    #             attackers_in_subchannel = [sender for sender, _, _ in users if isinstance(sender, (RLAttacker, FixAttacker))]
    #             has_attacker = len(attackers_in_subchannel) > 0
    #             normal_users = [user for user in users if not isinstance(user[0], (RLAttacker, FixAttacker))]
    #             normal_senders = [sender for sender, _, _ in normal_users]
    
    #             # 传统碰撞检测：多个发送者使用同一资源块
    #             collision_occurred = len(normal_users) > 1
    
    #             # 初始化发送者的预期接收计数
    #             for sender, packet, _ in users:
    #                 if not isinstance(sender, (RLAttacker, FixAttacker)):
    #                     # 计算应该接收此包的接收者数量
    #                     expected_receivers = sum(1 for receiver in self.vehicles 
    #                                             if receiver.id != sender.id 
    #                                             and receiver.should_receive_packet(sender.position))
    #                     sender.expected_receptions += expected_receivers
    ...
    #                         if receiver.should_receive_packet(sender.position):
    #                             # 创建消息键
    #                             msg_key = (sender.id, receiver.id, packet.packet_id)
    
    #                             # 如果消息不存在，创建新消息
    #                             if msg_key not in self.message_pool:
    #                                 self.message_pool[msg_key] = Message(sender.id, receiver.id, packet.packet_id)
    ...
    #                                 if other_sender.id != sender.id:
    #                                     interferers_pos.append(other_sender.position)
    #                                     interferer_ids.append(other_sender.id)
    ...
    #                             # 记录接收情况到消息
    #                             self.message_pool[msg_key].record_reception(success)
    
    #                             # 处理接收计数：
    #                             # 1. 所有接收者都进行物理接收（包括攻击者监听）
    #                             # 2. 但只有正常车辆发送给正常车辆的包才计入统计
                                
    #                             # 攻击者总是接收所有包（用于监听）
    #                             if isinstance(receiver, (RLAttacker, FixAttacker)):
    #                                 receiver.receive_packet(packet, resource, success)
                                
    #                             # 正常车辆接收包的处理
    #                             elif not isinstance(sender, (RLAttacker, FixAttacker)):
    #                                 # 正常车辆接收正常车辆发送的包
    #                                 receiver.receive_packet(packet, resource, success)
    ...
    #                             # 正常车辆接收攻击者包（不调用receive_packet，即不计入统计）
    
    #                             # 如果失败且有攻击者，且发送者是正常车辆，记录攻击成功
    #                             if (not success and has_attacker 
    #                                     and not isinstance(sender, (RLAttacker, FixAttacker))):
    #                                 collision_info['collisions_caused'] += 1
    #                                 self.attack_transmission_count += 1
    
    #                                 # 只为实际参与干扰的攻击者记录攻击成功
    #                                 for attacker in attackers_in_subchannel:
    #                                     if isinstance(attacker, RLAttacker):
    #                                         attacker.record_attack_success(True)
    #                                     elif isinstance(attacker, FixAttacker):
    #                                         attacker.collisions_caused += 1
    ...
    #             # 如果不使用SINR，使用传统碰撞检测
    #             else:
    #                 for sender, packet, resource in users:  # 处理所有用户（包括攻击者）
    #                     for receiver in self.vehicles:
    #                         if receiver.id != sender.id and receiver.should_receive_packet(sender.position):
    #                             # 创建消息键
    #                             msg_key = (sender.id, receiver.id, packet.packet_id)
    ...
    #                             if isinstance(sender, (RLAttacker, FixAttacker)):
    #                                 # 攻击者发送的包：正常车辆不计入统计
    #                                 success = False  # 物理层可能成功，但上层不计入
    #                                 if isinstance(receiver, (RLAttacker, FixAttacker)):
    #                                     # 攻击者接收其他攻击者的包
    #                                     receiver.receive_packet(packet, resource, True)
    #                             else:
    #                                 # 正常发送者
    #                                 success = not (has_attacker or collision_occurred)
    #                                 receiver.receive_packet(packet, resource, success)
    ...
    #         # 接收者消息状态字典 - 格式: {(sender_id, packet_id, receiver_id): status}
    #         # 每个接收者对于每个消息的状态
    #         receiver_message_status = defaultdict(lambda: {
    #             'resources_used': 0,
    #             'success': True,  # 初始假设成功
    ...
    #                              # 处理接收
    #                             if isinstance(receiver, Vehicle):
    #                                 # 这里不需要再调用，因为已经在receiver_message_status中处理了
    #                                 receiver.receive_packet(packet, resource, success)
    #                             else:
    #                                 if isinstance(sender, (RLAttacker, FixAttacker)):
    #                                     pRsvp = 20
    ...
    #                                 receiver_message_status[msg_key]['failed_resources'].add((slot_id, subchannel))
    ...
    #                             'success': True,
    #                             'failed_resources': set(),
    #                             'expected': True  # 标记为预期接收者
    #                         }
                    
    #                 # 更新消息的资源块计数（如果没有SINR计算）
    #                 if not self.use_sinr:
    #                     for receiver_id in packet.expected_receivers:
    #                         msg_key = (sender.id, packet.packet_id, receiver_id)
    #                         receiver_message_status[msg_key]['resources_used'] += 1
    ...
    #                 sender = next((v for v in self.vehicles if v.id == sender_id), None)
    #                 if sender and not isinstance(sender, (RLAttacker, FixAttacker)):
    #                     # 如果消息失败且接收者是预期的，增加发送者碰撞计数
    #                     if not status['success'] and status['expected']:
    #                         sender.collisions += 1
                    
    #                 # 更新全局统计
    #                 if status['expected']:  # 只统计预期接收者
    #                     self.total_expected_packets += 1
    #                     if status['success']:
    #                         self.total_received_packets += 1
    #                     else:
    ...
    #                 if not status['success'] and status['expected']:
    #                     # 查找导致失败的资源块
    #                     for resource_id in status['failed_resources']:
    #                         slot, subch = resource_id
    #                         users = subchannel_usage.get(subch, [])
    #                         for attacker in [a for a in self.attackers if a.id in [s.id for s,_,_ in users]]:
    #                             attacker.collisions_caused += 1
    #                             attacker.record_attack_success(True)
